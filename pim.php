...
$q = "
SELECT * FROM pim WHERE projectid = '$projectid'";

$r = mysql_query($q);
while ($row = mysql_fetch_array($r))
{	
	$target_pim = $row['target_pim'];
}
...
$page_title = "Project Impact Matrix Form";
				...
				<article class="col-sm-12 col-md-12 col-lg-12">
					...
					<div class="jarviswidget jarviswidget-color-teal" id="wid-id-1" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false">
						...
						<header>
							...
							<h2>Project Impact Matrix Form</h2>
						...
						<div>
							...
							<div class="widget-body no-padding">
								...
								<form enctype="multipart/form-data"  class="smart-form" method="post" action="process_pim.php">
								<input type="hidden" name="projectid" value="<?php echo $projectid; ?>">

									<fieldset>
							
										<section>
											<label class="label"><h2>Project Impact Matrix Form</h2></label>
											<div class="note">
												<strong>Note:</strong> 
												Please upload the project impact matrix form.
											 </div>
											<div class="input input-file">
											<input type="hidden" name="MAX_FILE_SIZE" value="10000000" />
												<span class="button"><input type="file" id="file" name="file_pim" onchange="this.parentNode.nextSibling.value = this.value">Browse</span><input type="text" placeholder="Upload Form" readonly="">
											</div>	
											<a href="<?php echo $target_pim; ?>">Save/View Uploaded Form</a>
										</section>
								
									</fieldset>
									<footer>
										<button type="submit" class="btn btn-primary" name="button" value="upload">
											Upload
										</button>
									</footer>
								</form>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//CKEDITOR.replace( 'background', { height: '100px'} );
		//CKEDITOR.replace( 'objectives', { height: '100px'} );
		//CKEDITOR.replace( 'customerrequirement', { height: '100px'} );
		//CKEDITOR.replace( 'projectimpact', { height: '100px'} );
		CKEDITOR.replace( 'methodology', { height: '100px'} );
		//CKEDITOR.replace( 'scope', { height: '100px'} );
		//CKEDITOR.replace( 'deliverables', { height: '100px'} );
		//CKEDITOR.replace( 'criticalpath', { height: '100px'} );
		//CKEDITOR.replace( 'milestones', { height: '100px'} );
		//CKEDITOR.replace( 'humanresource', { height: '100px'} );
		//CKEDITOR.replace( 'facilities', { height: '100px'} );
		//CKEDITOR.replace( 'statutory', { height: '100px'} );
